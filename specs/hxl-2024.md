...
**HXL** is a data interchange format similar to JSON or YAML, but with a couple
of additional features, such as references and inheritance.
...
A HXL file consists of one or more **nodes**.
...
To indicate a reference, the name of the property must be suffixed with
``&``.
...
Inheritance means a node will take over the properties from another node,
and only override the values which are different.
...
In this example, ``MonsterTwo`` implicitly gets ``health`` set to 100.
...
- **Numerics**
    ...
    Float values must be given on integer form, or with a decimal followed be AT LEAST one digit
      (example: ``-5.05``)
...
- **Arrays**
    ...
    - [``HXL_ARRAY_UNKNOWN_TYPE``] Array values MUST be either string or numeric.
...
Properties NOT specified in the schema must cause an error.
...
**Tokenization** is breaking up the HXL file at a very primitive level, where
no real attention is paid to _what_ you're breaking apart, or the order in which
is happens.

In the **grammar validation** you verify that the order tokens are provided
in, is in accordance with the expectations.
...
In the final stage, the **schema validation**, you hold the structure up
...
multiple specifications, and the client can select which specification
they adhere to.
...
| ``HXL_ILLEGAL_WHITESPACE``    | Wrong type of whitespace, or unallowed omission.                    |
...
| ``HXL_ARRAY_UNKNOWN_TYPE``      | When anything other than string or numeric is found in an array                                                                                                   |
