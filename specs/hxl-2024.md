...
- **Strings**:
    ...
    - [``HXL_ILLEGAL_STRING``] Within a string literal ``\n`` is NOT allowed (unless escaped).
- **Integer**
...
- **Floating-point values**:
    - [``HXL_ILLEGAL_FLOAT``] Float values are ONLY valid when a decimal point and at least one decimal are present.
      (example: ``-5.05``)
...
- **Arrays**
    ...
    - [``HXL_ARRAY_UNKNOWN_TYPE``] Array values MUST be either string, integer or floating-point values.
    - Nested arrays and references are NOT supported.
- **References**
    - Property key must be suffixed with ``&``.
    - [``HXL_ILLEGAL_WHITESPACE``] There must be NO whitespace surrounding the ``&`` (example ``key&: value``).
...
Node names and property keys are unique.
...
| ``HXL_ARRAY_UNKNOWN_TYPE``       | When anything other than string, integer or floating-point values are found in an array                                                                           |
...
| ``HXL_ILLEGAL_FLOAT``            | The format of the floating-point value is not allowed (most likely given as integer)                                                                              |
| ``HXL_ILLEGAL_STRING``           | The string contains illegal characters, such as ``\n``.                                                                                                           |
| ``HXL_NON_UNIQUE_NODE``          | When a node name is used 2+ times.                                                                                                                                |
| ``HXL_NON_UNIQUE_PROPERTY``      | When a node contains 2+ properties with identical keys.                                                                                                           |
